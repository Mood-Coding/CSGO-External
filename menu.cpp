...
void Menu::AddItem(string name, string* display, int& value, int max)
{
...
void Menu::Render()
{
	...
	ScreenToClient(this->OverlayHWND, &this->c.pos);

	// navegação
	this->MouseSetup();
	/*this->MouseScrollNavigation(x + tabWidth + 5, y + 25, w - tabWidth - 20, (this->scrollInfo.num * 16));*/
	this->MouseHoldScrollBarNavigation(x + w - 10, y + 27, h - 15, this->c.pos);
	...
	draw.Text(x + w/2, y + 2, this->menuInfo.title.c_str(), OffColor, false, TextAlignment::kCenter);
...
// TODO sửa lại cái này thành dùng chuột để kéo scrollbar
...
void Menu::EmptyScrollBarDragDirectionCheck(POINT currentPos)
{
	// Check dragDirection
	if (this->scrollInfoEx.dragDirection == "")
	{
		std::cout << "check 000\n";
		// cursor is moving down
		if (currentPos.y > this->scrollInfoEx.firstTimePos.y)
		{
			this->scrollInfoEx.dragDirection = "down";
			std::cout << "change\n";
		}
		...
		if (currentPos.y < this->scrollInfoEx.firstTimePos.y)
		{
			this->scrollInfoEx.dragDirection = "up";
			std::cout << "change\n";
		}
	}
}

void Menu::ScrollBarNextDragDirectionCheck(POINT currentPos)
{
	if (currentPos.y == this->scrollInfoEx.lastTimePos.y)
		return;

	std::string nextScrollBarDragDirection = "";

	if (currentPos.y > this->scrollInfoEx.lastTimePos.y)
		nextScrollBarDragDirection = "down";
	else
		nextScrollBarDragDirection = "up";

	// Change scrollbar drag direction
	if (nextScrollBarDragDirection != this->scrollInfoEx.dragDirection)
	{
		this->scrollInfoEx.dragDirection = nextScrollBarDragDirection;
		this->scrollInfoEx.firstTimePos = currentPos;
		this->scrollInfoEx.moveIndex = 0;
	}
}

void Menu::MouseHoldScrollBarNavigation(int x, int y, int h, POINT currentPos)
{
	...
	if (this->IsMouseOver(x - 2, y + indexOffset, 6, scrollHeight) &&
		this->IsHolding())
	{
		if (this->scrollInfoEx.isFirstTimeCheck)
		{
			this->scrollInfoEx.firstTimePos = currentPos;

			this->scrollInfoEx.isDragging = true; 
			this->scrollInfoEx.isFirstTimeCheck = false;
		}
		else
		{
			this->EmptyScrollBarDragDirectionCheck(currentPos);

			// Check if drag direction is changed
			this->ScrollBarNextDragDirectionCheck(currentPos);

			this->scrollInfoEx.lastTimePos = currentPos;

			int cursorOffset = this->scrollInfoEx.lastTimePos.y - this->scrollInfoEx.firstTimePos.y;

			// cursor isn't staying
			if (cursorOffset != 0 && scrollOffset != 0)
			{
				int nextMoveIndex = cursorOffset / scrollOffset;

				// cursor is moving down
				if(cursorOffset > 0)
				{
					if (nextMoveIndex != this->scrollInfoEx.moveIndex && this->scrollInfo.index < this->NumScrollItems())
					{
						this->scrollInfo.index++;
						this->itemInfo.index++;
						this->scrollInfoEx.moveIndex = nextMoveIndex;
					}
				}
				// cursor is moving up
				else if (nextMoveIndex != this->scrollInfoEx.moveIndex && this->scrollInfo.index > 0)
				{
					this->scrollInfo.index--;
					this->itemInfo.index--;
					this->scrollInfoEx.moveIndex = nextMoveIndex;
	...
	// Not dragging ScrollBar anymore
	if (!this->IsHolding())
	{
		this->scrollInfoEx.isDragging = false;
		this->scrollInfoEx.isFirstTimeCheck = true;
		this->scrollInfoEx.dragDirection = "";
		this->scrollInfoEx.moveIndex = 0;
	}
}
...
void Menu::DrawTabs(int x, int y, int w, int h)
{
	for (int i = 0; i < this->tabInfo.num; i++)
	{
		...
		draw.Text(x + w/2, y + (i * h) + (h/2) - 8, this->tabs[i].name.c_str(), color, false, TextAlignment::kCenter);
	}
}

bool Menu::MouseNavigation(int itemIndex)
{
	if (this->IsLClicked())
	{
...
void Menu::DrawItems(int x, int y, int w)
{
	...
	for (int i = this->FirstScrollIndex(); i <= this->LastScrollIndex(); i++)
	{
		...
		// lista de opções
		draw.Text(x + w - w/4 - 30, itemOffset + 5, this->items[i].display[this->items[i].value].c_str(), color, false, TextAlignment::kRight);

		if (this->IsMouseOver(x - 5, itemOffset + 5, w - w/4 - 20, 16))
		{
			if (this->MouseNavigation(i))
