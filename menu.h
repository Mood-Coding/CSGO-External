...
public:
	...
	void AddItem(string name, string* display, int& value, int max = 2);
...
private:
	void MouseSetup();
	bool MouseNavigation(int itemIndex);
	/*void MouseScrollNavigation(float x, float y, float w, float h);*/
	void KeyboardNavigation();
	void MouseHoldScrollBarNavigation(int x, int y, int h, POINT currentPos);
	...
	struct ScrollInfoEx { int moveIndex = 0; bool isDragging = false; bool isFirstTimeCheck = true; POINT firstTimePos; POINT lastTimePos; std::string dragDirection = ""; } scrollInfoEx;
	...
	void EmptyScrollBarDragDirectionCheck(POINT currentPos);
	void ScrollBarNextDragDirectionCheck(POINT currentPos);
